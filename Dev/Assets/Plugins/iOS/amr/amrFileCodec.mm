...
int sizes[] = { 17, 23, 32, 36, 40, 46, 50, 58, 60, 5, -1, -1, -1, -1, -1, 0 };
...
// 返回值: 0-出错; 1-正确
int ReadAMRFrameData_WB(const char* fpamr,int pos,int maxLen, unsigned char frameBuffer[])
{
    int bytes = 0;
    int nSize = 0;
    unsigned char frameHeader; // 帧头
    ...
    if (pos >=maxLen) {
        NSLog(@"read amr frame end0");
        return 0;
    }
    // 读帧头
    frameHeader = fpamr[pos];
    
    // 读该帧的语音数据(帧头已经读过)
    frameBuffer[0] = frameHeader;
    int stdFrameSize = sizes[(frameHeader >> 3) & 0x0f];
    nSize = stdFrameSize*sizeof(unsigned char);
    if (nSize < 0) {
        NSLog(@"read amr   end1");
        return 0;
    }
    if (pos+nSize >= maxLen) {
        NSLog(@"read amr frame end2");
        return 0;
    }
    memcpy(&(frameBuffer[1]), fpamr+pos+1, nSize);
...
void WriteWAVEHeader(NSMutableData* fpwave, int nFrame)
{
    ...
    + nFrame*PCM_FRAME_SIZE*sizeof(short);    //
	strcpy(tag, "WAVE");
	...
	wfx.nFormatTag = 1;
	wfx.nChannels = 1; // 单声道
	wfx.nSamplesPerSec = AMR_RATE_HZ; // 8khz
	wfx.nAvgBytesPerSec = AMR_RATE_HZ*2;
	wfx.nBlockAlign = 2;
	...
	strcpy(tag, "data");
	memcpy(chunk.chChunkID, tag, 4);
    chunk.nChunkSize = nFrame*PCM_FRAME_SIZE*sizeof(short);
	//fwrite(&chunk, 1, sizeof(XCHUNKHEADER), fpwave);
    [fpwave appendBytes:&chunk length:sizeof(XCHUNKHEADER)];
...
NSData* DecodeAMRToWAVE_WB(NSData* data) {
    ...
    unsigned char amrFrame[500];
    unsigned char littleendian[640];
    unsigned char *ptr;
    short pcmFrame[320];
    ...
    while(1)
    {
        ...
        nTemp = ReadAMRFrameData_WB(rfile,pos,maxLen, amrFrame);
        if (!nTemp)
        {
            NSLog(@"read amr file end");
            bErr = 1;
            break;
        }
        pos += nTemp;
        ...
        ptr = littleendian;
        for (i = 0; i < 320; i++) {
            *ptr++ = (pcmFrame[i] >> 0) & 0xff;
            *ptr++ = (pcmFrame[i] >> 8) & 0xff;
        }
        [fpwave appendBytes:littleendian length:640];
    }
    NSLog(@"frame = %d", nFrameCount);
    D_IF_exit(destate);
    WriteWAVEHeader(out, nFrameCount);
...
NSData* DecodeAMRToWAVE(NSData* data) {
    if (AMR_RATE_HZ == 8000)
    {
        DecodeAMRToWAVE_NB(data);
    }
    else if (AMR_RATE_HZ == 16000)
    {
        DecodeAMRToWAVE_WB(data);
    }
}
...
// 从WAVE文件读一个完整的PCM音频帧
// 返回值: 0-错误 >0: 完整帧大小
int ReadPCMFrameData_NB(short speech[], char* fpwave, int nChannels, int nBitsPerSample)
{
	int nRead = 0;
	int x = 0, y=0;
...
// 从WAVE文件读一个完整的PCM音频帧
// 返回值: 0-错误 >0: 完整帧大小
int ReadPCMFrameData_WB(short speech[], char* fpwave, int nChannels, int nBitsPerSample)
{
    int nRead = 0;
    int x = 0, y=0;
    unsigned short ush1=0, ush2=0, ush=0;
    ...
    nRead = (nBitsPerSample/16) * PCM_FRAME_SIZE*nChannels;
    if (nBitsPerSample==8 && nChannels==1)
    {
        memcpy(pcmFrame_8b1,fpwave,nRead);
        for(x=0; x<PCM_FRAME_SIZE; x++)
...
NSData* EncodePCMToAMR_NB(char* data, int maxLen,int nChannels, int nBitsPerSample)
{
    /*
    char* oldBuf = data;
    // input speech vector
	...
	while(1)
		...
		int nRead = ReadPCMFrameData_NB(speech, data, nChannels, nBitsPerSample);
        data += nRead;
		frames++;
		// call encoder
...
// bps = 8 --> 8位 unsigned char
//       16 --> 16位 unsigned short
NSData* EncodePCMToAMR_WB(char* data, int maxLen,int nChannels, int nBitsPerSample)
{
    char* oldBuf = data;
    /* input speech vector */
    ...
    {
        // read one pcm frame
        if ((data-oldBuf+640)>maxLen) {
            break;
        }
        int nRead = ReadPCMFrameData_WB(speech, data, nChannels, nBitsPerSample);
        data += nRead;
        ...
        /* call encoder */
        byte_counter = E_IF_encode(enstate, req_mode, speech, amrFrame, 0);
...
NSData* EncodePCMToAMR(char* data, int maxLen,int nChannels, int nBitsPerSample)
{
    if (AMR_RATE_HZ == 8000)
    {
        return EncodePCMToAMR_NB(data, maxLen, nChannels, nBitsPerSample);
    }
    else if (AMR_RATE_HZ == 16000)
    {
        return EncodePCMToAMR_WB(data, maxLen, nChannels, nBitsPerSample);
    }
}
