	...
	public class XCBuildConfiguration : PBXObject
	{
		...
		{	
			...
			foreach( string path in paths ) {
                ...
                {
                    ((PBXList)((PBXDictionary)_data[BUILDSETTINGS_KEY])[key]).Remove(currentPath);
                }
                else if (currentPath.IndexOf("\"") == -1 && ((PBXList)((PBXDictionary)_data[BUILDSETTINGS_KEY])[key]).Contains("\"" + currentPath + "\""))
                {
                    ((PBXList)((PBXDictionary)_data[BUILDSETTINGS_KEY])[key]).Remove("\"" + currentPath + "\"");
                }
                else if (currentPath.IndexOf("\\\"") == -1 && ((PBXList)((PBXDictionary)_data[BUILDSETTINGS_KEY])[key]).Contains("\\\"" + currentPath + "\\\""))
