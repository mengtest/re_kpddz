...
public class UIAtlasMaker : EditorWindow
{
	...
	void OnSelectAtlas (Object obj)
	{
		if (NGUISettings.atlas != obj)
		{
            ...
            if (NGUISettings.atlas != null)
            {
                ...
                {
                    Debug.LogError("找不到" + NGUISettings.atlas.name + "的图片!");
                }
            }
			Repaint();
		}
		...
		if (Selection.objects != null && Selection.objects.Length > 0)
		{
			...
			foreach (Object o in objects)
			{
				...
				if (NGUISettings.atlas == null || NGUISettings.atlas.texture != tex)
                    if (tex.name.IndexOf("_Alpha") == -1 && tex.name.IndexOf("_RGB") == -1) textures.Add(tex);
			}
		}
		return textures;
	}
	...
	static public void ExtractSprites (UIAtlas atlas, List<SpriteEntry> finalSprites)
	{
        ...
        //Temp Use Base Png as mainTexture
        SetBasePngAsAtlasTexture();
        ...
        //Back To RGB as mainTexture;
        SetRGBPngAsAtlasTexture();
		ShowProgress(1f);
	}
	...
	static public bool UpdateTexture (UIAtlas atlas, List<SpriteEntry> sprites)
	{
        ...
        oldPath = oldPath.Replace("_RGB.png", ".png");
        oldPath = oldPath.Replace("_Alpha.png", ".png");
        newPath = newPath.Replace("_RGB.png", ".png");
        newPath = newPath.Replace("_Alpha.png", ".png");
        ...
        bool newTexture = (tex == null || oldPath != newPath || !System.IO.File.Exists(newPath));
		...
		if (PackTextures(tex, sprites))
		{
            ...
            MaterialTextureForETC1.SeperateRGBAandlphaChannel(newPath);
    ...
    static string SetBasePngAsAtlasTexture()
    {
        string rgb_path = "";
        //Temp Use Base Png as mainTexture
        if (NGUISettings.atlas != null)
        {
            ...
            else if (NGUISettings.atlas.texture.name.IndexOf("_RGB") != -1)
            {
                Texture2D tex_now = NGUISettings.atlas.texture as Texture2D;
                string pngPath = (tex_now != null) ? AssetDatabase.GetAssetPath(tex_now.GetInstanceID()) : "";
                if (!string.IsNullOrEmpty(pngPath))
                {
                    rgb_path = pngPath;
                    pngPath = pngPath.Replace("_RGB", "");
                    Texture2D tex_png = NGUIEditorTools.ImportTexture(pngPath, true, false, false);
                    if (tex_png != null)
                    {
                        NGUISettings.atlas.texture = tex_png;
                    }
                    else
                    {
                        Debug.LogError("找不到图片:" + pngPath);
                    }
                }
            }
            else if (NGUISettings.atlas.texture.name.IndexOf("_Alpha") != -1)
            {
                Texture2D tex_now = NGUISettings.atlas.texture as Texture2D;
                string pngPath = (tex_now != null) ? AssetDatabase.GetAssetPath(tex_now.GetInstanceID()) : "";
                if (!string.IsNullOrEmpty(pngPath))
                {
                    rgb_path = pngPath.Replace("_Alpha", "_RGB");
                    pngPath = pngPath.Replace("_Alpha", "");
                    Texture2D tex_png = NGUIEditorTools.ImportTexture(pngPath, true, false, false);
                    if (tex_png != null)
                    {
                        NGUISettings.atlas.texture = tex_png;
    ...
    static void SetRGBPngAsAtlasTexture()
    {
        string rgb_path = "";
        //Back To RGB as mainTexture;
        if (NGUISettings.atlas != null && NGUISettings.atlas.texture != null)
        {
            if (NGUISettings.atlas.texture.name.IndexOf("_RGB") != -1)
            {
                //NGUISettings.atlas.texture = NGUIEditorTools.ImportTexture(rgb_path, true, false, false);
            }
            else if (NGUISettings.atlas.texture.name.IndexOf("_Alpha") != -1)
            {
                Texture2D tex_now = NGUISettings.atlas.texture as Texture2D;
                string pngPath = (tex_now != null) ? AssetDatabase.GetAssetPath(tex_now.GetInstanceID()) : "";
                rgb_path = pngPath.Replace("_Alpha", "_RGB");
                Texture2D tex_rgb = NGUIEditorTools.ImportTexture(rgb_path, true, false, false);
                if (tex_rgb != null)
                {
                    NGUISettings.atlas.texture = tex_rgb;
                }
                else
                {
                    Debug.LogError("找不到图片:" + rgb_path);
                }
            }
            else
            {
                Texture2D tex_now = NGUISettings.atlas.texture as Texture2D;
                string pngPath = (tex_now != null) ? AssetDatabase.GetAssetPath(tex_now.GetInstanceID()) : "";
                rgb_path = pngPath.Replace(".png", "_RGB.png");
                Texture2D tex_rgb = NGUIEditorTools.ImportTexture(rgb_path, true, false, false);
                if (tex_rgb != null)
                {
                NGUISettings.atlas.texture = tex_rgb;
	...
	void OnGUI ()
	{
		...
		if (NGUISettings.atlas != null)
		{
			...
			if (mat != null)
			{
				...
				if (shader != null)
				{
                    if (shader.name == "Unlit/Transparent Colored" || shader.name == "Unlit/Transparent Colored Alpha")
					{
