...
#define NGUI_TEXTURE_ALPHA_SPLIT
...
public class UIDrawCall : MonoBehaviour
{
    ...
    Texture         mAlphaTexture;  // Alpha texture used by the material
    ...
    /// <summary>
	/// Alpha texture used by the material.
	/// </summary>
    ...
    void CreateMaterialAlphaSplit()
    {
        //非ALPHA分离采用原来的方法创建材质
        //这种区分方式比较简单粗暴，通过名字来区分，当没有alpha分离的shader用类似的命名可能会出现UI绘制错误
        //TODO: 优化区分方式 @WP.Chu
        if (mShader != null && !mShader.name.EndsWith(" Alpha"))
        {
            CreateMaterial();
            return;
        }
        ...
        // Legacy functionality
        const string soft = " (SoftClip) Alpha";
        shaderName = shaderName.Replace(soft, "");

        const string textureClip = " (TextureClip) Alpha";
        shaderName = shaderName.Replace(textureClip, "");

        if (panel.clipping == Clipping.TextureMask)
        {
            if (shaderName.Contains(" Alpha"))
        ...
        else if (mClipCount != 0)
        {
            ...
            if (shaderName.Contains("Unlit/Text"))
            {
                shader = Shader.Find("Hidden/" + shaderName + " " + mClipCount);
                if (shader == null) shader = Shader.Find(shaderName + " " + mClipCount);
            }
            ...
            {
                shader = Shader.Find("Hidden/" + shaderName + " " + mClipCount + " Alpha");
                if (shader == null) shader = Shader.Find(shaderName + " " + mClipCount + " Alpha");
            }
    ...
    Material RebuildMaterial ()
	{
...
#if NGUI_TEXTURE_ALPHA_SPLIT
        CreateMaterialAlphaSplit();
#else
        CreateMaterial();
#endif
        ...
        //Assign the alpha texture if exist
        if (mAlphaTexture != null && mDynamicMat.HasProperty("_AlphaTex"))
            mDynamicMat.SetTexture("_AlphaTex", mAlphaTexture);
	...
	void SetClipping (int index, Vector4 cr, Vector2 soft, float angle)
	{
		...
		{
            Vector4 range = new Vector4(-cr.x / cr.z, -cr.y / cr.w, 1f / cr.z, 1f/ cr.w);
            //Vector4 arg = new Vector4(sharpness.x, sharpness.y, Mathf.Sin(angle), Mathf.Cos(angle));
            Vector2 arg = new Vector4(sharpness.x, sharpness.y);
			mDynamicMat.SetVector(ClipRange[index], range);
            mDynamicMat.SetVector(ClipArgs[index], arg);
            //Utils.LogSys.Log("Range:(" + range.x.ToString() + ", " + range.y.ToString() + ", " + range.z.ToString() + ", " + range.w.ToString() + "),    Arg:" + arg.ToString());
		}
	}
	...
	static public UIDrawCall Create (UIPanel panel, Material mat, Texture tex, Shader shader, Texture alphaTex = null)
	{
#if UNITY_EDITOR
		string name = null;
		...
		return Create(name, panel, mat, tex, shader, alphaTex);
#else
		return Create(null, panel, mat, tex, shader, alphaTex);
#endif
    }
    ...
    static UIDrawCall Create (string name, UIPanel pan, Material mat, Texture tex, Shader shader, Texture alphaTex = null)
	{
		UIDrawCall dc = Create(name);
		dc.gameObject.layer = pan.cachedGameObject.layer;
		dc.baseMaterial = mat;
		dc.mainTexture = tex;
        dc.alphaTexture = alphaTex;
        dc.shader = shader;
