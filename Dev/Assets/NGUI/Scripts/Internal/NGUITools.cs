...
//#endif
    ...
    static private Rect GetCameraAdjustRect()
    {
        //       Vector2 screen = NGUITools.screenSize;
        Vector2 screen = new Vector2(Screen.width, Screen.height);
        float aspect = screen.x / screen.y;//CalculateAspect(screen.x, screen.y);//设备宽高比值
        float initialAspect = 1136f / 640f;//理想的宽高比值

        if (Math.Abs(aspect - initialAspect) < 0.01f)
        {
            //基本一样时，不用调整
        }
        if (aspect > initialAspect)//镜头宽度的缩小
        {
            float scaleX = screen.y * initialAspect / screen.x;
            return new Rect((1f - scaleX)/2f, 0f, scaleX, 1f);
        }
        else if (aspect < initialAspect)//镜头高度要缩小
        {
            float scaleY = screen.x / (initialAspect * screen.y);
            return new Rect(0f, (1f - scaleY) / 2f, 1f, scaleY);
        }
