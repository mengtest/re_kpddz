...
#region Fill Functions
	...
	void SlicedFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
        ...
        if (mTempPos[3].x - mTempPos[0].x < br.x)//只要显示左
        {
            mTempUVs[1].x = mTempPos[3].x - mTempPos[0].x;
        }
        else if (mTempPos[3].x - mTempPos[0].x < br.x + br.z)//只要显示左+中
        {
            mTempUVs[3].x = mTempPos[3].x - mTempPos[0].x - br.x;
        }
		for (int x = 0; x < 3; ++x)
		{
			int x2 = x + 1;
